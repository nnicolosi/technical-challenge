...
const ContactsPage = () => {
  ...
  //Added a useState hook for editRow. Set as an integer to signify which row is being edited.
    ...
    getAllContacts().then((response) => {
      if (response.data && response.data.length) {
        ...
        const rows = response.data.map((row, rowIndex) => (
          ...
          <td>
            <button type="submit" className="button is-secondary" disabled={formDisabled}>FINISH</button>
          </td>
        </tr> 
        :
          <tr key={rowIndex}>
